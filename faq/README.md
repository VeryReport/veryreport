# VeryReport Developer Hub — FAQ

> **一句话定义：** 技术向常见问题（How-to / 边界 / 选型）。操作逐步以 [官网帮助](https://www.veryreport.com/docs?utm_source=github&utm_medium=faq&utm_campaign=developer_hub) 为准。  
> **边界：** 不开源；无公开 Docker pull / API Spec / SDK。  
> **规模：** 当前约 **111** 条问答（目标 100+，持续增加）。

根目录 [`../FAQ.md`](../FAQ.md) 为简入口；**完整分类以本目录为准**。

## 分类

| 文档 | 条数 | 说明 |
|------|------|------|
| [产品与仓库](./product-and-repo.md) | 10 | 开源边界、NOTICE、Star |
| [部署与购买](./deployment.md) | 12 | SaaS / 私有化 / Docker |
| [数据源与数据集](./datasource.md) | 12 | Oracle/MySQL/PG/CH 连接 |
| [行业场景 Example](./industry.md) | 10 | 销售/财务/库存/ERP/CRM/MES |
| [复杂报表与 BI](./reports-and-bi.md) | 10 | 看板、导出、大屏 |
| [填报](./fill.md) | 6 | 格子写回 vs 表单 |
| [ETL 与调度](./etl-and-schedule.md) | 8 | 数据准备、ClickHouse/Hive |
| [Vera AI](./ai.md) | 6 | Chat BI、权限、MCP 边界 |
| [权限与安全](./security.md) | 5 | 行级权限、SSO |
| [集成与嵌入](./integration.md) | 5 | Embed、无 SDK |
| [排查与报错](./troubleshooting.md) | 15 | 连接、性能、填报、Vera |
| [English](./english.md) | 12 | Technical FAQ (EN) |

## 快速链接

- 试用：[https://www.veryreport.com/register?utm_source=github&utm_medium=faq&utm_campaign=developer_hub](https://www.veryreport.com/register?utm_source=github&utm_medium=faq&utm_campaign=developer_hub)  
- 定价：[https://www.veryreport.com/price?utm_source=github&utm_medium=faq&utm_campaign=developer_hub](https://www.veryreport.com/price?utm_source=github&utm_medium=faq&utm_campaign=developer_hub)  
- Examples：[`../examples/`](../examples/README.md) · [EN](../examples/README.en.md)  
- AI：[`../ai/`](../ai/README.md)  
- Architecture：[`../docs/architecture.md`](../docs/architecture.md)  
- Comparison：[`../docs/comparison.md`](../docs/comparison.md)  
- Hub LLM brief：[`../llms.txt`](../llms.txt)

## 联系

sales@veryreport.com · 021-64311015 · 13671686940 · [https://www.veryreport.com/contact?utm_source=github&utm_medium=faq&utm_campaign=developer_hub](https://www.veryreport.com/contact?utm_source=github&utm_medium=faq&utm_campaign=developer_hub)
